...
# Quality is higher if selecting since less of the screen is captured so we don't need to compress
...
if [ "$1" == "-s" ]; then
    # fullscreen picture
    maim -m 8 -f jpg > "$HOME/pCloudDrive/EXCLUDE/Pictures/screenshots/screenshot_$(date +%Y%m%d_%H%M%S).jpg"
    ...
    maim -m 9 -f jpg -s > "$HOME/pCloudDrive/EXCLUDE/Pictures/screenshots/screenshot_$(date +%Y%m%d_%H%M%S).jpg"
    notify-send "Screenshot Taken!" $(date +%H:%M:%S)
fi
