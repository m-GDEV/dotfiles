...
while [ 7 -gt 6 ]; do
    ...
    echo "$(date) | $CPU - $(( TEMP / 1000))" >> cpu.log

    MAX_TEMP=$(cat ./cpu.log | awk '{print $11}' | sort -n | tail -1)
    MIN_TEMP=$(cat ./cpu.log | awk '{print $11}' | sort -n | head -1)
    ...
    NUMS=$(cat ./cpu.log | wc -l)
   
    # TEMPERATURES
    TEMPS=$(cat ./cpu.log | awk '{print $11}')
    for i in $TEMPS; do TOTAL_TEMP=$(($TOTAL_TEMP + $i)); done
    AVG_TEMP=$(($TOTAL_TEMP / $NUMS))

    # PERCENTAGES
    PERS=$(cat ./cpu.log | awk '{print $9}' | cut -c 1-2 | tr '.' ' ' | tr '%' ' ')
